...
//        method.invoke() //
    ...
    private boolean auth(Security security, String name){
        if (security != null){
            String[] auth = security.value();
            List<String> authUser = new ArrayList<>(Arrays.asList(auth));
            System.out.println(authUser);
            if (!authUser.contains(name)){
                return false;
            }
        }
        return true;
